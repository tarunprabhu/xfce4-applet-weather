           ...
           data_types type)
{
    ...
    str = g_strdup_printf("%3s%s", value, "°");
    g_free(value);
    return str;
}
...
void
update_valuebox(plugin_data *data,
                 gboolean immediately)
{
    ...
    if (data->weatherdata && data->weatherdata->current_conditions) {
        label = make_label(data, TEMPERATURE);
        gtk_label_set_text(GTK_LABEL(data->valuebox), label);
        g_free(label);
        weather_debug("Added temperature to value box.");
    } else {
      gtk_label_set_text(GTK_LABEL(data->valuebox), "---");
      weather_debug("No weather data available, set single label '%s'.", "---");
    }
    weather_debug("Updated value box.");
}
                          ...
                          gboolean immediately)
{
    ...
    /* update widgets */
    update_icon(data);
    update_valuebox(data, immediately);
...
schedule_next_wakeup(plugin_data *data)
{
    ...
    if (!strcmp(data->next_wakeup_reason, "regular check"))
        weather_debug("[%s]: Running regular check for updates, "
                      "interval %d secs.", date, data->update_interval);
    else {
                        ...
                        plugin_data *data)
{
    ...
    value = xfce_rc_read_entry(rc, "valuebox_font", NULL);
    if (value) {
        g_free(data->valuebox_font);
        data->valuebox_font = g_strdup(value);
    }

    value = xfce_rc_read_entry(rc, "valuebox_color", NULL);
    if (value)
        gdk_color_parse(value, &(data->valuebox_color));

    data->update_interval =
        xfce_rc_read_int_entry(rc, "update_interval", DEFAULT_UPDATE_INTERVAL);
                         ...
                         plugin_data *data)
{
    ...
    if (data->valuebox_font)
        xfce_rc_write_entry(rc, "valuebox_font", data->valuebox_font);

    value = gdk_color_to_string(&(data->valuebox_color));
    xfce_rc_write_entry(rc, "valuebox_color", value);
    g_free(value);
           ...
           gpointer user_data)
{
    ...
    g_signal_handlers_block_by_func(data->eventbox, cb_toggled, data);
    if (data->summary_window)
        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(data->eventbox), TRUE);
    else
        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(data->eventbox), FALSE);
    g_signal_handlers_unblock_by_func(data->eventbox, cb_toggled, data);
    return FALSE;
}
...
{
    xml_time *conditions;
    gchar *text, *sym, *alt, *temp, *apparent;
    gchar *windspeed, *windbeau, *winddir, *winddeg;
    gchar *pressure, *humidity, *precipitation;
    gchar *fog, *cloudiness, *value;
    gchar *point, *interval_start, *interval_end;
    gchar *sunrise = NULL, *sunset = NULL;
    const gchar *unit;
    ...
    if (data->current_astro)
        if (data->current_astro->sun_never_rises) {
            sunrise = g_strdup(_("The sun never rises today."));
            sunset = g_strdup("");
        } else if (data->current_astro->sun_never_sets) {
            sunrise = g_strdup("");
            sunset = g_strdup(_("The sun never sets today."));
        } else {
    ...
    DATA_AND_UNIT(apparent, APPARENT_TEMPERATURE);
    ...
    case TOOLTIP_SIMPLE:
        text = g_markup_printf_escaped
            ...
            (_("<b><span size=\"large\">%s</span></b> "
             ...
             temp, apparent, windspeed, winddir, pressure, humidity);
        break;
    ...
    default:
        text = g_markup_printf_escaped
            ...
            (_("<b><span size=\"large\">%s</span></b> "
               ...
               "</span>\n\n"
               "<b>Temperature:</b> %s <span size=\"smaller\">(%s)</span>\t\t"
               "<span size=\"smaller\">[Updated at %s]</span>\n"
               "<b>Wind:</b> %s (%s on the Beaufort scale) from %s(%s)\n"
               "<b>Pressure:</b> %s\t\t"
               "<b>Humidity:</b> %s\n"
               "<b>Fog:</b> %s\t\t\t\t"
               "<b>Cloud:</b> %s\n\n"
               "<span size=\"smaller\">"
               "<b>Sunrise:</b> %s\t\t\t\t<b>Sunset:</b> %s"
               "</span>"),
             data->location_name, alt, 
             translate_desc(sym, data->night_time),
...
xfceweather_create_control(XfcePanelPlugin *plugin)
{
    ...
    data->valuebox = gtk_label_new(NULL);
    ...
    gtk_container_add(GTK_CONTAINER(plugin), data->eventbox);
    xfce_panel_plugin_add_action_widget(plugin, data->eventbox);
    gtk_widget_show(data->eventbox);
    ...
    gtk_container_add(GTK_CONTAINER(data->eventbox), data->alignbox);

    /* add widgets to alignment box */
    data->vbox_center_valuebox = gtk_vbox_new(FALSE, 0);
    gtk_misc_set_alignment(GTK_MISC(data->iconimage), 1, 0.5);
    gtk_box_pack_start(GTK_BOX(data->alignbox),
                       data->iconimage, TRUE, FALSE, 2);
    gtk_box_pack_start(GTK_BOX(data->vbox_center_valuebox),
                       data->valuebox, TRUE, TRUE, 0);
    gtk_box_pack_start(GTK_BOX(data->alignbox),
                       data->vbox_center_valuebox, TRUE, TRUE, 2);
    gtk_widget_show_all(data->alignbox);

    /* hook up events for the button */
    g_object_set(G_OBJECT(data->eventbox), "has-tooltip", TRUE, NULL);
    g_signal_connect(G_OBJECT(data->eventbox), "query-tooltip",
                     G_CALLBACK(weather_get_tooltip_cb), data);
    g_signal_connect(G_OBJECT(data->eventbox), "button-press-event",
                     G_CALLBACK(cb_click), data);
    g_signal_connect(G_OBJECT(data->eventbox), "toggled",
                     G_CALLBACK(cb_toggled), data);
    gtk_widget_add_events(data->valuebox, GDK_BUTTON_PRESS_MASK);
...
weather_construct(XfcePanelPlugin *plugin)
{
    ...
    gtk_widget_modify_font(
        GTK_WIDGET(data->valuebox),
        pango_font_description_from_string(data->valuebox_font));
    gtk_widget_modify_fg(GTK_WIDGET(data->valuebox),
                         GTK_STATE_NORMAL,
                         &data->valuebox_color);
